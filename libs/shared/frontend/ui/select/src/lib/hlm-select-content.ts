...
import { classes, hlm, type HlmOverlayWidth } from '@spartan-ng/helm/utils';
...
export class HlmSelectContent {
	...
	constructor() {
		classes(() => [
			'bg-popover no-scrollbar text-popover-foreground data-open:animate-in data-closed:animate-out data-closed:fade-out-0 data-open:fade-in-0 data-closed:zoom-out-95 data-open:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 ring-foreground/10 max-h-72 flex-col rounded-lg shadow-md ring-1 duration-100 relative flex overflow-x-hidden overflow-y-auto',
			this.width() === 'content'
				? 'w-max min-w-(--brn-select-width) max-w-[calc(100vw-2rem)] sm:max-w-lg'
				: 'w-(--brn-select-width) max-w-[calc(100vw-2rem)]',
		]);
	}
}
