...
- **Vector Database**: PostgreSQL with pgvector - Stores versioned legal-source chunks and embeddings.
...
### Phase 3: Routing & Legal Knowledge Retrieval

- **Switch Node**: Routes the flow based on the `job_type` from the JSON.
- **Database Query**: The system queries PostgreSQL `pgvector` for versioned Serbian legal-source chunks and article metadata according to the request.
