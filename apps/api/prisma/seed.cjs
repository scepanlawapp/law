...
async function provisionTenantSchema(prisma, schemaName) {
  const domainStatements = [
    ...[
      ...
      (type) =>
        `DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type t JOIN pg_namespace n ON n.oid = t.typnamespace WHERE t.typname = '${type}' AND n.nspname = '${schemaName}') THEN CREATE TYPE "${schemaName}"."${type}" AS ENUM (${type === "ClientType" ? "'INDIVIDUAL', 'ORGANIZATION'" : type === "ClientStatus" ? "'ACTIVE', 'INACTIVE', 'ARCHIVED', 'PROSPECT'" : type === "ClientContactStatus" ? "'ACTIVE', 'INACTIVE'" : type === "ActivityType" ? "'NOTE', 'PHONE_CALL', 'MEETING', 'EMAIL', 'OTHER'" : type === "ActivitySource" ? "'MANUAL', 'SYSTEM', 'AI'" : type === "CaseStatus" ? "'DRAFT', 'ACTIVE', 'ON_HOLD', 'CLOSED', 'ARCHIVED'" : "'LOW', 'NORMAL', 'HIGH', 'URGENT'"}); END IF; END $$;`,
    ),
    ...
    `CREATE TABLE IF NOT EXISTS "${schemaName}"."Client" ("id" TEXT NOT NULL, "workspaceId" TEXT NOT NULL, "clientNumber" TEXT NOT NULL, "type" "${schemaName}"."ClientType" NOT NULL, "displayName" TEXT NOT NULL, "firstName" TEXT, "lastName" TEXT, "organizationName" TEXT, "isDomestic" BOOLEAN NOT NULL DEFAULT true, "jmbg" TEXT, "taxNumber" TEXT, "registrationNumber" TEXT, "status" "${schemaName}"."ClientStatus" NOT NULL DEFAULT 'ACTIVE', "email" TEXT, "phone" TEXT, "website" TEXT, "preferredLanguage" TEXT, "notes" TEXT, "customFields" JSONB, "responsibleUserId" TEXT, "createdByUserId" TEXT NOT NULL, "updatedByUserId" TEXT NOT NULL, "createdAt" TIMESTAMPTZ(3) NOT NULL DEFAULT CURRENT_TIMESTAMP, "updatedAt" TIMESTAMPTZ(3) NOT NULL DEFAULT CURRENT_TIMESTAMP, CONSTRAINT "Client_pkey" PRIMARY KEY ("id"), CONSTRAINT "Client_workspaceId_clientNumber_key" UNIQUE ("workspaceId", "clientNumber"))`,
    `CREATE TABLE IF NOT EXISTS "${schemaName}"."ClientAddress" ("id" TEXT NOT NULL, "clientId" TEXT NOT NULL, "addressType" TEXT NOT NULL, "street" TEXT NOT NULL, "streetAdditional" TEXT, "city" TEXT NOT NULL, "postalCode" TEXT NOT NULL, "stateOrRegion" TEXT, "country" TEXT NOT NULL, "note" TEXT, "isPrimary" BOOLEAN NOT NULL DEFAULT false, "createdAt" TIMESTAMPTZ(3) NOT NULL DEFAULT CURRENT_TIMESTAMP, "updatedAt" TIMESTAMPTZ(3) NOT NULL DEFAULT CURRENT_TIMESTAMP, CONSTRAINT "ClientAddress_pkey" PRIMARY KEY ("id"), CONSTRAINT "ClientAddress_clientId_fkey" FOREIGN KEY ("clientId") REFERENCES "${schemaName}"."Client"("id") ON DELETE CASCADE)`,
    `CREATE TABLE IF NOT EXISTS "${schemaName}"."ClientIdentificationDocument" ("id" TEXT NOT NULL, "clientId" TEXT NOT NULL, "type" TEXT NOT NULL, "number" TEXT NOT NULL, "issuedDate" DATE, "expiredDate" DATE, "country" TEXT NOT NULL, "createdAt" TIMESTAMPTZ(3) NOT NULL DEFAULT CURRENT_TIMESTAMP, "updatedAt" TIMESTAMPTZ(3) NOT NULL DEFAULT CURRENT_TIMESTAMP, CONSTRAINT "ClientIdentificationDocument_pkey" PRIMARY KEY ("id"), CONSTRAINT "ClientIdentificationDocument_clientId_fkey" FOREIGN KEY ("clientId") REFERENCES "${schemaName}"."Client"("id") ON DELETE CASCADE)`,
    ...
    `CREATE INDEX IF NOT EXISTS "ClientIdentificationDocument_clientId_createdAt_idx" ON "${schemaName}"."ClientIdentificationDocument"("clientId", "createdAt")`,
...
async function main() {
  ...
  try {
    ...
    if (!user) {
      user = await platformPrisma.user.create({
        data: {
          email,
      ...
      user = await platformPrisma.user.update({
        where: { id: user.id },
        data: {
          passwordHash: hashPassword(password),
          status: "ACTIVE",
    ...
    await platformPrisma.workspaceMember.upsert({
      where: {
