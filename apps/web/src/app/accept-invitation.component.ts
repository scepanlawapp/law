...
@Component({
  ...
  template: `
    <main class="auth-shell"><section class="auth-panel">
      <p class="eyebrow">Law workspace</p><h1>Accept invitation</h1>
      <form (ngSubmit)="submit()">
        <label>Password <input name="password" type="password" [(ngModel)]="password" minlength="12" required autocomplete="new-password"></label>
        @if (error()) { <p class="error" role="alert">{{ error() }}</p> }
        @if (complete()) { <p role="status">Your account is ready. Redirecting to sign in.</p> }
        <button type="submit" [disabled]="submitting() || complete()">{{ submitting() ? "Activating..." : "Set password" }}</button>
      </form>
    </section></main>
  `,
  styles: [`
...
export class AcceptInvitationComponent {
  ...
  submit(): void {
    ...
    this.api.acceptInvitation({ token: this.token, password: this.password }).subscribe({
      next: () => { this.complete.set(true); setTimeout(() => void this.router.navigate(["/login"]), 800); },
      error: () => { this.error.set("This invitation is invalid or expired."); this.submitting.set(false); },
    });
  }
}