...
## Sidebar default usage

The Sidebar family is generated locally when the workspace does not already contain it. The generated entry point is `@spartan-ng/helm/sidebar`, which exports `HlmSidebarImports`, `HlmSidebarService`, and `provideHlmSidebarConfig`. The primitive uses its own responsive sheet behavior on mobile and persists desktop open/collapsed state through a cookie.

Use the wrapper around the sidebar and the main content. Import `HlmSidebarImports` into both standalone components when they own sidebar primitives:
        ...
        <div hlmSidebarGroupContent>
          <ul hlmSidebarMenu>
            <li hlmSidebarMenuItem>
              <a hlmSidebarMenuButton routerLink="/clients" routerLinkActive #active="routerLinkActive" [isActive]="active.isActive" tooltip="Clients">
                <span>Clients</span>
              </a>
            </li>
          </ul>
...
Use `collapsible="none"` for a permanently expanded desktop sidebar. Use `HlmSidebarService.toggleSidebar()` or `hlmSidebarTrigger` for toggling; do not recreate the mobile overlay, keyboard shortcut, cookie persistence, or collapsed tooltip behavior in application code. If defaults need to change, provide `provideHlmSidebarConfig({...})` at the appropriate application/layout injector.
